...
static std::uintptr_t get_module_base(const DWORD pid, const wchar_t* moduel_name) {
	...
	if (Module32FirstW(snap_shot, &entry) == TRUE) {
		if (wcsstr(moduel_name, entry.szModule) != nullptr)
...
int main() {
	...
	const HANDLE driver = CreateFile(L"\\\\.\\KernelDriver", GENERIC_READ, 0, nullptr, OPEN_EXISTING,
	...
	if (driver::attach_to_process(driver, pid) == true) {
		...
		if (const std::uintptr_t client = get_module_base(pid, L"client.dll"); client != 0) {
			...
			while (true) {
				...
				const auto local_entList = driver::read_memory<std::uintptr_t>(driver, client + cs2_dumper::offsets::client_dll::dwEntityList);
				...
				const auto force_jump =
					driver::read_memory<DWORD>(driver, client + cs2_dumper::buttons::jump);
				...
				auto entityList = driver::read_memory<std::uintptr_t>(driver, client + cs2_dumper::offsets::client_dll::dwEntityList);
			}
		}
	}
